...
const demoToggleBtn = document.querySelector("#demoToggleBtn");
...
const YOUTUBE_DEMO_STORAGE_KEY = "youtubeDemoEnabled";
const YOUTUBE_DEMO_SCRIPT_VERSION = globalThis.chrome?.runtime?.getManifest?.().version || "0.2.1";
...
let youtubeDemoEnabled = false;
const analysisCache = new Map();

renderEmptyState();
void initializeYouTubeDemoMode();

demoToggleBtn.addEventListener("click", async () => {
  if (!hasExtensionApi() || !globalThis.chrome?.storage?.local) {
    return;
  }

  const nextEnabled = !youtubeDemoEnabled;
  demoToggleBtn.disabled = true;
  demoStatus.textContent = nextEnabled ? "데모 영상을 준비하고 있습니다." : "원래 YouTube 화면으로 복원하고 있습니다.";

  try {
    await chrome.storage.local.set({
      [YOUTUBE_DEMO_STORAGE_KEY]: nextEnabled,
    });
    youtubeDemoEnabled = nextEnabled;

    await synchronizeYouTubeDemoWithActiveTab(nextEnabled);

    renderYouTubeDemoState();
  } catch (error) {
    demoStatus.textContent = `데모 모드 변경 실패: ${getFriendlyError(error)}`;
  } finally {
...
async function initializeYouTubeDemoMode() {
  if (!hasExtensionApi() || !globalThis.chrome?.storage?.local) {
    demoToggleBtn.disabled = true;
    demoToggleBtn.textContent = "사용 불가";
    demoStatus.textContent = "확장 프로그램으로 실행할 때 사용할 수 있습니다.";
    return;
  }

  try {
    const stored = await chrome.storage.local.get(YOUTUBE_DEMO_STORAGE_KEY);
    youtubeDemoEnabled = Boolean(stored[YOUTUBE_DEMO_STORAGE_KEY]);
    renderYouTubeDemoState();
    await synchronizeYouTubeDemoWithActiveTab(youtubeDemoEnabled);
  } catch (error) {
    demoStatus.textContent = `데모 상태 확인 실패: ${getFriendlyError(error)}`;
  }
}

async function synchronizeYouTubeDemoWithActiveTab(enabled) {
  ...
  if (enabled) {
    await injectYouTubeDemoIntoTab(tab.id);
    return;
  }
  ...
  try {
    const [probe] = await chrome.scripting.executeScript({
      ...
      func: () => Boolean(document.querySelector([
        "#deepfake-signal-demo-player",
        "[data-deepfake-signal-demo-card]",
        "[data-deepfake-signal-demo-underlay]",
      ].join(", "))),
    });
    if (probe?.result) {
    ...
    // The content script may already be reloading the tab after demo mode is disabled.
  }
}

async function injectYouTubeDemoIntoTab(tabId) {
  try {
    const [probe] = await chrome.scripting.executeScript({
      target: { tabId },
      func: (version) => document.documentElement?.getAttribute(
        "data-deepfake-signal-youtube-demo",
      ) === version,
      args: [YOUTUBE_DEMO_SCRIPT_VERSION],
    });
    if (probe?.result) {
      return;
    }
  } catch {
    // Continue with injection when the page is still initializing.
  }

  const results = await Promise.allSettled([
    chrome.scripting.insertCSS({
      target: { tabId },
      files: ["youtube_demo.css"],
    }),
    chrome.scripting.executeScript({
      target: { tabId },
      files: ["youtube_demo.js"],
    }),
  ]);
...
function renderYouTubeDemoState() {
  demoToggleBtn.setAttribute("aria-pressed", String(youtubeDemoEnabled));
  demoToggleBtn.textContent = youtubeDemoEnabled ? "켜짐" : "꺼짐";
  demoStatus.textContent = youtubeDemoEnabled
    ? "YouTube 홈에서는 첫 번째 데모 카드만 실행됩니다."
    : "홈 화면의 원래 링크를 그대로 사용합니다.";
}

function isYouTubeUrl(value) {
  try {
    return ["youtube.com", "www.youtube.com"].includes(new URL(value).hostname);
  } catch {
    return false;
  }
}
